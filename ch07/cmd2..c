 ...
 * each command.
 *
 * This program extends cmd1 to demonstrate the use of setjmp and longjmp.
 */
#include "apue.h"
#include <setjmp.h>
...
int main(void) {
  ...
  if (setjmp(jmpbuffer) != 0) { /* called directly; expect return value = 0 */
    printf("error");
  }
...
void cmd_add(void) {
  ...
  if (token < 0) { /* an error has occurred */
    /* second argument is non-zero return value from setjmp() in main() */
    longjmp(jmpbuffer, 1);
  }
